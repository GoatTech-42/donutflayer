    ...
    <div class="auth-banner">
      ...
      <div class="auth-code-box">
        <div>
          ...
          <div class="auth-code-value">${esc(data.code)}</div>
        </div>
      </div>
      <div class="auth-url">
        Open <a href="${esc(data.fullUrl)}" target="_blank">${esc(data.url)}</a> in your browser and enter the code above.
      </div>
      <div class="auth-timer">Expires in ${data.expiresIn || 600}s</div>
    </div>
  `;
}
